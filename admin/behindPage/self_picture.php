...
$sql = "select id from program where poster='$poster'";
...
$sql = "SELECT * FROM program where poster='$poster' LIMIT ".($page-1)*$pagesize .",$pagesize";
$sql2 = "SELECT * FROM program where poster='$poster' LIMIT ".($page-1)*$pagesize2 .",$pagesize2";
...
<script type="text/javascript" language="javascript" src="../../js/self.js"></script>
...
<div id="panelwrap">
	...
	<div class="header">
    	...
    	<div class="menu">
    <ul>
    <li><a href="#manage" class="selected">管理程序</a></li>
    <li><a onclick="a_a()" style="cursor:pointer">添加程序</a></li>
...
$page_banner.="<form action = 'self_program.php' method='get'>";
...
?>
    ...
    <td><?php echo $i; $i++;?></td>
            <td>&nbsp;<a href="article.show.php?id=<?php echo $value['id'];?>&table=program"><?php echo $value['name']?></a></td>
            <td>&nbsp;<?php echo $size?></td>
            <td>&nbsp;<?php  if($value['self']==0) echo "公开";else echo "不公开";?></td>
            <td><a href="app.modify.php?id=<?php echo $value['id'];?>&table=program" >修改</a></td>  
            <td><a href="app.del.handle.php?id=<?php echo $value['id'];?>&table=program" onclick="javascript:return confir()">删除</a></td>  <?php 
...
?>
	...
	?>
    	<tr class="odd">
            <td>&nbsp;<?php echo $i;$i++;?></td>
            <td>&nbsp;<a href="article.show.php?id=<?php echo $value['id'];?>&table=program" title="<?php echo $value['description'];?>"><?php  $length=mb_strlen($value['name'],"utf8");if($length<20) echo $value['name'];else echo mb_substr($value['name'],0,20,"utf8")."...";?></a></td>
			<td><?php echo $size;?></td>
            <td>&nbsp;<?php if($value['self']==0) echo "公开";else echo "不公开";?></td>
    ...
    <form id="form1" name="form1" method="post" action="deal_pictureUp.php" enctype="multipart/form-data">
        ...
        <div class="form">
            ...
            <div class="form_row">
            请选择附件
			 <input type="file" name="myfile" class="view" id="myfile" onchange="pre(myfile)" />
			 <input type="submit" class="form_submit" value="Submit" onclick="return check(form1)" />
             <input type="hidden" name="table" value="program" />
            </div> 
            <div id="P" ><img   name="preview" id="preview" onload="img_size()" src="" /></div>
            <div class="clear"></div>
        </div>
        </form>
    </div> 
